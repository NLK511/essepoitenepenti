...
class SentimentSnapshotResolver:
    ...
    def resolve_macro_snapshot(self) -> dict[str, Any]:
        ...
        payload = self._snapshot_payload(snapshot) if snapshot is not None else {
            ...
            "source_breakdown": {},
            "drivers": [],
            "diagnostics": {"warnings": ["macro sentiment snapshot unavailable; using context-only fallback"]},
        }
    ...
    def resolve_industry_snapshot(self, ticker: str) -> dict[str, Any]:
        ...
        payload = self._snapshot_payload(snapshot) if snapshot is not None else {
            ...
            "diagnostics": {
                "warnings": [
                    f"industry sentiment snapshot unavailable for {industry_profile['subject_label']}; using context-only fallback"
                ]
            },
        }
        if context_snapshot is not None:
