...
class WatchlistCalibrationReviewService:
    ...
    def calibration_review(
        ...
        *,
        ...
        buckets = self._review_buckets(
            ...
            horizon_setup_key=horizon_setup_key,
        )
        ...
        return {
            ...
            "setup_family": self.bucket_snapshot(setup_family, buckets["setup_family"]),
            "confidence_bucket": self.bucket_snapshot(bucket_key, buckets["confidence_bucket"]),
            "horizon": self.bucket_snapshot(horizon, buckets["horizon"]),
            "transmission_bias": self.bucket_snapshot(transmission_bias, buckets["transmission_bias"]),
            "context_regime": self.bucket_snapshot(context_regime, buckets["context_regime"]),
            "horizon_setup_family": self.bucket_snapshot(horizon_setup_key, buckets["horizon_setup_family"]),
    ...
    def _review_buckets(
        ...
        *,
        ...
        context_regime: str,
        horizon_setup_key: str,
    ) -> dict[str, object | None]:
        return {
            "setup_family": self.find_calibration_bucket(getattr(calibration_summary, "by_setup_family", []), setup_family),
            "confidence_bucket": self.find_calibration_bucket(getattr(calibration_summary, "by_confidence_bucket", []), bucket_key),
            "horizon": self.find_calibration_bucket(getattr(calibration_summary, "by_horizon", []), horizon),
            "transmission_bias": self.find_calibration_bucket(getattr(calibration_summary, "by_transmission_bias", []), transmission_bias),
            "context_regime": self.find_calibration_bucket(getattr(calibration_summary, "by_context_regime", []), context_regime),
            "horizon_setup_family": self.find_calibration_bucket(getattr(calibration_summary, "by_horizon_setup_family", []), horizon_setup_key),
        }

    def _bucket_review_adjustments(
        self,
        calibration_summary: object,
        confidence_percent: float,
        *,
        ...
        for label, bucket, hard_penalty, soft_penalty, reward, hard_conf_penalty, soft_conf_penalty, conf_reward in self._reviewed_bucket_specs(buckets):
            ...
            confidence_adjustment += self.bucket_confidence_adjustment(
                label,
                bucket,
                overall_win_rate=overall_win_rate,
    ...
    @staticmethod
    def _reviewed_bucket_specs(buckets: dict[str, object | None]) -> tuple[tuple[str, object | None, float, float, float, float, float, float], ...]:
        return (
            ("setup_family", buckets["setup_family"], 10.0, 5.0, -2.0, 1.6, 0.9, -0.6),
            ("confidence_bucket", buckets["confidence_bucket"], 10.0, 5.0, -2.0, 1.2, 0.75, -0.5),
            ("horizon", buckets["horizon"], 4.0, 2.0, -1.0, 0.85, 0.5, -0.3),
            ("transmission_bias", buckets["transmission_bias"], 3.0, 1.5, -0.75, 0.6, 0.35, -0.2),
            ("context_regime", buckets["context_regime"], 3.0, 1.5, -0.75, 0.6, 0.35, -0.2),
            ("horizon_setup_family", buckets["horizon_setup_family"], 4.0, 2.0, -1.0, 0.75, 0.4, -0.2),
        )
