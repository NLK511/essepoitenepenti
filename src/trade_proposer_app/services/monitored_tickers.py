...
from trade_proposer_app.persistence.models import BrokerOrderExecutionRecord, BrokerPositionRecord, WatchlistRecord
...
class MonitoredTickerService:
    ...
    def list_monitored_tickers_with_provenance(self) -> list[dict[str, object]]:
        ...
        for record in self.session.scalars(select(BrokerPositionRecord)).all():
            if str(record.status or "").strip().lower() in self.ACTIVE_POSITION_STATUSES and float(record.current_quantity or 0.0) > 0.0:
                self._add(provenance, record.ticker, "broker_position")
        return [{"ticker": ticker, "provenance": sorted(values)} for ticker, values in sorted(provenance.items())]
