...
class SettingsRepository:
    ...
    def get_steering_config(self) -> dict[str, object]:
        ...
        return {
            ...
            "cancel_expired_pending_orders_enabled": self._get_bool(setting_map, "steering_cancel_expired_pending_orders_enabled", True),
            "cancel_invalidated_pending_orders_enabled": self._get_bool(setting_map, "steering_cancel_invalidated_pending_orders_enabled", True),
            "move_to_profit_enabled": self._get_bool(setting_map, "steering_move_to_profit_enabled", True),
            "close_on_severe_invalidation_enabled": self._get_bool(setting_map, "steering_close_on_severe_invalidation_enabled", True),
            "tighten_on_deterioration_enabled": self._get_bool(setting_map, "steering_tighten_on_deterioration_enabled", True),
            "lower_tp_on_weakness_enabled": self._get_bool(setting_map, "steering_lower_tp_on_weakness_enabled", True),
            "pending_expiration_grace_minutes": self._get_int(setting_map, "steering_pending_expiration_grace_minutes", 5),
            "pending_min_confidence_percent": self._get_float(setting_map, "steering_pending_min_confidence_percent", 55.0),
            "pending_invalidation_required_signals": self._get_int(setting_map, "steering_pending_invalidation_required_signals", 2),
            "pending_price_chase_limit_percent": self._get_float(setting_map, "steering_pending_price_chase_limit_percent", 1.0),
            "breakeven_trigger_percent": self._get_float(setting_map, "steering_breakeven_trigger_percent", 0.75),
            "min_profit_lock_percent": self._get_float(setting_map, "steering_min_profit_lock_percent", 0.10),
            "position_close_confidence_percent": self._get_float(setting_map, "steering_position_close_confidence_percent", 40.0),
            "position_close_required_signals": self._get_int(setting_map, "steering_position_close_required_signals", 3),
            "position_min_hold_confidence_percent": self._get_float(setting_map, "steering_position_min_hold_confidence_percent", 50.0),
            "position_deterioration_required_signals": self._get_int(setting_map, "steering_position_deterioration_required_signals", 2),
            "deterioration_stop_cushion_percent": self._get_float(setting_map, "steering_deterioration_stop_cushion_percent", 0.35),
            "weakened_thesis_tp_cushion_percent": self._get_float(setting_map, "steering_weakened_thesis_tp_cushion_percent", 0.50),
            "min_tp_distance_percent": self._get_float(setting_map, "steering_min_tp_distance_percent", 0.10),
            "min_reviewed_dry_run_decisions_before_enable": self._get_int(setting_map, "steering_min_reviewed_dry_run_decisions_before_enable", 30),
            "min_reviewed_dry_run_amendments_before_enable": self._get_int(setting_map, "steering_min_reviewed_dry_run_amendments_before_enable", 10),
            "min_reviewed_dry_run_close_now_before_enable": self._get_int(setting_map, "steering_min_reviewed_dry_run_close_now_before_enable", 10),
        }
    ...
    def set_steering_config(
        ...
        *,
        ...
        min_reviewed_dry_run_decisions_before_enable: int,
        min_reviewed_dry_run_amendments_before_enable: int,
        min_reviewed_dry_run_close_now_before_enable: int,
    ) -> dict[str, object]:
        self.set_settings(
            {
                "steering_enabled": str(bool(enabled)).lower(),
                "steering_dry_run": str(bool(dry_run)).lower(),
                "steering_cancel_expired_pending_orders_enabled": str(bool(cancel_expired_pending_orders_enabled)).lower(),
                "steering_cancel_invalidated_pending_orders_enabled": str(bool(cancel_invalidated_pending_orders_enabled)).lower(),
                "steering_move_to_profit_enabled": str(bool(move_to_profit_enabled)).lower(),
                "steering_close_on_severe_invalidation_enabled": str(bool(close_on_severe_invalidation_enabled)).lower(),
                "steering_tighten_on_deterioration_enabled": str(bool(tighten_on_deterioration_enabled)).lower(),
                "steering_lower_tp_on_weakness_enabled": str(bool(lower_tp_on_weakness_enabled)).lower(),
                "steering_pending_expiration_grace_minutes": str(max(0, int(pending_expiration_grace_minutes))),
                "steering_pending_min_confidence_percent": f"{float(pending_min_confidence_percent):.4f}".rstrip("0").rstrip("."),
                "steering_pending_invalidation_required_signals": str(max(0, int(pending_invalidation_required_signals))),
                "steering_pending_price_chase_limit_percent": f"{float(pending_price_chase_limit_percent):.4f}".rstrip("0").rstrip("."),
                "steering_breakeven_trigger_percent": f"{float(breakeven_trigger_percent):.4f}".rstrip("0").rstrip("."),
                "steering_min_profit_lock_percent": f"{float(min_profit_lock_percent):.4f}".rstrip("0").rstrip("."),
                "steering_position_close_confidence_percent": f"{float(position_close_confidence_percent):.4f}".rstrip("0").rstrip("."),
                "steering_position_close_required_signals": str(max(0, int(position_close_required_signals))),
                "steering_position_min_hold_confidence_percent": f"{float(position_min_hold_confidence_percent):.4f}".rstrip("0").rstrip("."),
                "steering_position_deterioration_required_signals": str(max(0, int(position_deterioration_required_signals))),
                "steering_deterioration_stop_cushion_percent": f"{float(deterioration_stop_cushion_percent):.4f}".rstrip("0").rstrip("."),
                "steering_weakened_thesis_tp_cushion_percent": f"{float(weakened_thesis_tp_cushion_percent):.4f}".rstrip("0").rstrip("."),
                "steering_min_tp_distance_percent": f"{float(min_tp_distance_percent):.4f}".rstrip("0").rstrip("."),
                "steering_min_reviewed_dry_run_decisions_before_enable": str(max(0, int(min_reviewed_dry_run_decisions_before_enable))),
                "steering_min_reviewed_dry_run_amendments_before_enable": str(max(0, int(min_reviewed_dry_run_amendments_before_enable))),
                "steering_min_reviewed_dry_run_close_now_before_enable": str(max(0, int(min_reviewed_dry_run_close_now_before_enable))),
            }
        )
