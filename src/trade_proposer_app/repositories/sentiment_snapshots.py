...
from trade_proposer_app.persistence.models import SentimentSnapshotRecord
...
class SentimentSnapshotRepository:
    ...
    def get_latest_snapshot(self, scope: str, subject_key: str) -> SentimentSnapshot | None:
        record = self.session.scalars(
            ...
            .where(SentimentSnapshotRecord.scope == scope)
            .where(SentimentSnapshotRecord.subject_key == subject_key)
            .order_by(SentimentSnapshotRecord.computed_at.desc())
            .limit(1)
        ).first()
        if record is None:
            return None
    ...
    def get_latest_valid_snapshot(self, scope: str, subject_key: str, now: datetime | None = None) -> SentimentSnapshot | None:
    ...
    def list_recent_snapshots(self, scope: str | None = None, limit: int = 50) -> list[SentimentSnapshot]:
        query = select(SentimentSnapshotRecord).order_by(SentimentSnapshotRecord.computed_at.desc()).limit(limit)
        if scope:
            query = query.where(SentimentSnapshotRecord.scope == scope)
        rows = self.session.scalars(query).all()
        return [self._to_model(row) for row in rows]

    @classmethod
    def _to_model(cls, record: SentimentSnapshotRecord) -> SentimentSnapshot:
