...
class RouteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dashboard_filters_by_window_and_keeps_only_proper_failures(self) -> None:
        ...
        self.assertIsNotNone(one_day_payload["dashboard_summary"]["overall_win_rate_percent"])
        ...
        self.assertTrue(any(series["key"] == "average_profit_percent" for series in one_day_payload["dashboard_trends"]["series"]))
    ...
    async def test_dashboard_operator_status_returns_compact_gate_payload(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertIn("policy_trust", payload)
        self.assertIn("policy_health", payload)
        self.assertIn("edge_validation_gate", payload)
    ...
    async def test_broker_workbench_returns_orders_positions_and_risk(self) -> None:
        ...
        self.assertIn("broker_sync_state", payload)
        self.assertIn("last_at", payload["broker_sync_state"])
    ...
    async def test_recommendation_outcome_routes_and_plan_evaluation_queue_runs(self) -> None:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            actionability = await client.get("/api/recommendation-outcomes/actionability-diagnostics")
        ...
        self.assertEqual(actionability.status_code, 200)
        ...
        self.assertEqual(actionability.json()["actionable_resolved_outcomes"], 2)
        self.assertEqual(actionability.json()["phantom_resolved_outcomes"], 0)
        self.assertEqual(actionability.json()["actionability_gap_percent"], None)
        self.assertEqual(actionability.json()["diagnostic_source"], "simulation")
        self.assertIn("simulated_actionability_diagnostics", actionability.json())
