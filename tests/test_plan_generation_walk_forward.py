...
class PlanGenerationWalkForwardServiceTests(unittest.TestCase):
    ...
    def test_walk_forward_uses_slice_views_not_copied_lists(self) -> None:
        start = datetime(2026, 1, 1, tzinfo=timezone.utc)
        records = self._make_records(60, start)
        self.tuning_service._eligible_records.return_value = records
        seen_window_types: list[type] = []

        def score_side_effect(window, config):
            seen_window_types.append(type(window))
            self.assertNotIsInstance(window, list)
    ...
    def test_promotion_rejected_with_more_than_one_severe_regression(self) -> None:
        ...
        def score_side_effect(recs, config):
            ...
            if idx == 3:
                return (20, 8 if is_cand else 10, 0.4 if is_cand else 0.5, 0)
