...
def test_default_weekly_etoro_bar_shadow_comparison_job_is_scheduled() -> None:
    from trade_proposer_app.services.default_jobs import (
    ...
    spec = ensure_default_etoro_bar_shadow_comparison_job(session)

    assert spec["cron"] == "30 07 * * SAT"
    job = session.scalars(
        select(JobRecord).where(JobRecord.name == "Auto: eToro Bar Shadow Comparison Weekly")
    ).one()
    assert job.job_type == JobType.ETORO_BAR_SHADOW_COMPARISON.value
    assert job.schedule == "30 07 * * SAT"
    assert job.enabled is True
