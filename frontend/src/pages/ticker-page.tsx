  ...
  return (
    <>
      <PageHeader
        ...
        title={data ? `${data.ticker} recommendation-plan and outcome analysis` : "Ticker analysis"}
        subtitle="Use this page to inspect redesign-native recommendation plans for one ticker, the outcomes those plans reached after evaluation, and the underlying prototype trade outcomes."
        actions={
          <>
      ...
      {data ? (
        <div className="stack-page">
          <section className="metrics-grid">
            <Card><div className="metric-label">Stored plans</div><div className="metric-value">{data.performance.app_plan_count}</div></Card>
            <Card><div className="metric-label">Actionable plans</div><div className="metric-value">{data.performance.actionable_plan_count}</div></Card>
            <Card><div className="metric-label">Plan outcomes</div><div className="metric-value">{data.performance.win_plan_count} / {data.performance.loss_plan_count}</div></Card>
            <Card><div className="metric-label">Open plans</div><div className="metric-value">{data.performance.open_plan_count}</div></Card>
          ...
          <Card>
            ...
            <ul className="checklist">
              <li>Recommendation-plan counts and stored plan outcomes come from the redesign-native app database.</li>
              <li>Prototype WIN / LOSS / PENDING trade counts come from <code>{data.performance.prototype_trade_log_path}</code>.</li>
              <li>Running the evaluation workflow updates recommendation-plan outcomes from price-history evaluation rather than from legacy recommendation-detail state sync.</li>
              <li>Runs are execution records. Recommendation plans and outcomes are the canonical operator review objects.</li>
            </ul>
          </Card>

          <section className="card-grid">
            <Card>
              <SectionTitle kicker="Plan mix" title="App-side distribution" />
              <ul className="list-reset">
                <li className="list-item compact-item"><span>LONG</span><Badge tone="ok">{data.performance.long_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>SHORT</span><Badge tone="danger">{data.performance.short_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>NO_ACTION</span><Badge tone="neutral">{data.performance.no_action_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>WATCHLIST</span><Badge tone="neutral">{data.performance.watchlist_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>OPEN</span><Badge tone="warning">{data.performance.open_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>WIN</span><Badge tone="ok">{data.performance.win_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>LOSS</span><Badge tone="danger">{data.performance.loss_plan_count}</Badge></li>
                <li className="list-item compact-item"><span>Warnings</span><Badge tone={data.performance.warning_plan_count > 0 ? "warning" : "ok"}>{data.performance.warning_plan_count}</Badge></li>
              </ul>
            </Card>
            <Card>
                ...
                <table>
                  ...
                  <tbody>
                    {data.recommendation_plans.map((item) => {
                      ...
                      return (
                        <tr key={`${item.id}-${item.computed_at}`}>
                          <td>{formatDate(item.computed_at)}</td>
                          <td>
                            ...
                            <div className="helper-text top-gap-small">{item.confidence_percent}% confidence</div>
                            <div className="helper-text top-gap-small">{item.thesis_summary || item.rationale_summary || "No thesis summary stored."}</div>
                            <div className="helper-text top-gap-small"><Link to={`/jobs/recommendation-plans?ticker=${encodeURIComponent(item.ticker)}`}>Open plan browser</Link></div>
                          </td>
                          <td>{item.run_id ? <Link to={`/runs/${item.run_id}`}>#{item.run_id}</Link> : "—"}</td>
                          <td><div>Entry {item.entry_price_low ?? item.entry_price_high ?? "—"}{item.entry_price_high && item.entry_price_low && item.entry_price_high !== item.entry_price_low ? ` to ${item.entry_price_high}` : ""}</div><div>Stop {item.stop_loss ?? "—"}</div><div>Take profit {item.take_profit ?? "—"}</div></td>
                          <td>
                            <div className="cluster">
                            ...
                            <div className="helper-text top-gap-small">{item.latest_outcome?.notes || (item.warnings.length > 0 ? `${item.warnings.length} warning(s)` : "No warnings or outcome notes.")}</div>
                          </td>
                        </tr>
                      );
                    })}
