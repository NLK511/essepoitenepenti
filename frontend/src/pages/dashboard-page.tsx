...
export function DashboardPage() {
  const [searchParams, setSearchParams] = useSearchParams({ window: "1d" });
  ...
  async function loadDashboard({ clear = false, includeTrends = false }: { clear?: boolean; includeTrends?: boolean } = {}) {
    try {
      setError(null);
      if (clear) {
        setData(null);
      ...
      if (includeTrends && payload.dashboard_trends) {
        setDashboardTrends(payload.dashboard_trends);
      }
  ...
  async function loadDashboardTrends() {
    try {
      setError(null);
      const payload = await getJson<{ dashboard_trends: DashboardTrends }>("/api/dashboard/trends");
      setDashboardTrends(payload.dashboard_trends);
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : "Failed to load dashboard trendlines");
    }
  }
            ...
            <SectionTitle
              ...
              actions={
                <button
                  ...
                  onClick={() => {
                    const next = !showTrendlines;
                    setShowTrendlines(next);
                    if (next && !dashboardTrends) {
                      void loadDashboardTrends();
                    }
                  }}
                >
