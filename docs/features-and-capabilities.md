...
- Macro and industry context writers now dedupe repeated evidence, persist event lifecycle state (`new`, `escalating`, `persistent`, `fading`), and surface contradictory evidence directly in stored context metadata and run-detail operator review.
- Ticker deep analysis now consumes snapshot-plus-context enrichment so transmission logic can use active context events, regime tags, lifecycle state, and contradiction counts in addition to legacy sentiment scores.
- Watchlist orchestration now applies a bounded transmission-confidence adjustment and can emit explicit `context_transmission_contradiction` no-action plans when context evidence is too internally conflicted to trust.

Current limitation:
- macro and industry refresh runs now write richer context objects through a news-first event pipeline with deduping, lifecycle state, and contradiction handling, but the event model is still heuristic rather than a fully learned or fully curated causal engine
...
- recommendation-plan outcome persistence and evaluation are now first-class, watchlist-backed plan generation now records setup-family and decomposed confidence details, operator workflows can inspect grouped calibration summaries by confidence bucket and setup family, watchlist-backed action gating now applies calibration-aware confidence-threshold adjustments for underperforming buckets/setup families and now also folds in horizon, transmission-bias, context-regime, and horizon-plus-setup-family slices, recommendation-plan workflows now expose baseline cohort comparisons (actual actionable plans vs high-confidence, cheap-scan-attention, momentum-lane, and catalyst-lane slices) plus dedicated setup-family cohort reporting so breakout vs continuation vs mean-reversion behavior can be inspected directly, and recommendation-outcome workflows now expose a dedicated setup-family review surface with per-family horizon / transmission-bias / context-regime slices, direct setup-family filtering in the main recommendation-plan browse flow, and a new evidence-concentration view that highlights the strongest and weakest measurable cohorts; watchlist orchestration also carries richer ticker transmission summaries plus a small catalyst/event shortlist lane so purely technical ranking does not dominate every deep-analysis slot; those transmission summaries now include primary drivers, conflict flags, expected transmission windows, context-strength metrics, contradiction counts, and bounded transmission-confidence adjustments, while recommendation-plan generation now persists raw-vs-calibrated confidence, confidence adjustment, setup-family-specific entry style, stop style, target style, timing expectation, evaluation focus, action-reason detail, plus invalidation framing instead of only generic thesis text; run detail / recommendation-plan browse views surface them alongside transmission bias/alignment, action reasons, effective threshold context, and calibration sample status so recalibration remains visible rather than hidden
- legacy recommendation-detail and recommendation-history surfaces are retired from active operator workflow; new proposal-generation runs are redesign-native and should be reviewed through recommendation plans and outcomes, optimization now uses recommendation-plan outcomes, and the historical legacy `recommendations` table has been explicitly dropped from the active schema head
- the legacy `/api/history` recommendation-history API is retired from the mounted operator API surface, unused legacy history/detail frontend pages are removed, and evaluation-run summaries now report recommendation-plan outcome counts rather than legacy recommendation counters
