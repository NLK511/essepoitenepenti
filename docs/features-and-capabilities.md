...
- recommendation-plan outcome persistence and evaluation are now first-class, watchlist-backed plan generation now records setup-family and decomposed confidence details, and operator workflows can now inspect grouped calibration summaries by confidence bucket and setup family; confidence recalibration logic itself is still incomplete
