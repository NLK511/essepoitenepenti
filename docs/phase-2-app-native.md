...
- **Enhanced sentiment coverage**: `NaiveSentimentAnalyzer` now uses a broader keyword set (including hits like `guidance`, `exceed`, or `resilient demand`) while still honoring the signal integrity policy; headlines receive a stronger boost (1.7 vs 1.2) and the lighter smoothing factor (0.25) makes even single keyword matches nudge the compound score away from strict neutrality, while the `coverage_insights` list continues to explain every remaining zero.
