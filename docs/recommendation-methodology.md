...
- the redesign path now has first-class recommendation outcome persistence/evaluation, watchlist-backed plans now persist setup family plus decomposed confidence components, grouped calibration summaries are available by confidence bucket and setup family, watchlist-backed action gating now applies early calibration-aware threshold adjustments for underperforming buckets/setup families, operator workflows now compare actual actionable plans against simple heuristic baseline cohorts, and ticker planning now persists richer macro/industry transmission summaries while shortlist construction reserves a small catalyst/event lane alongside the main technical lane; operator views now surface lane selection, transmission bias/alignment, action reasons, and effective threshold context directly; true recalibration is still incomplete
