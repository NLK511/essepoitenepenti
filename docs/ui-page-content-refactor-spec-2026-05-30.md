...
10. Refocus tuning pages around evidence-backed action. **In progress:** Signal gating tuning now leads with the upstream-selection question, links back to Quality & Edge, puts dry-run/apply controls in the first decision card, demotes manual parameter edits behind an advanced disclosure, and collapses stored run summary JSON.
