...
- **Enhanced sentiment coverage**: `NaiveSentimentAnalyzer` now inspects a broader keyword set but keeps the signal integrity policy in place so runs with no matched keywords remain zero and expose their missing coverage instead of relying on fallback heuristics.
