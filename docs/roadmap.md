...
- **Enhanced sentiment coverage**: `NaiveSentimentAnalyzer` now inspects a broader keyword set, gives headlines a slightly larger boost (1.7 vs 1.2) while the summary hits are still rewarded, and uses a tighter smoothing constant (0.25) so even sparse matches nudge the compound score away from zero while the signal integrity policy keeps coverage gaps explicit.
